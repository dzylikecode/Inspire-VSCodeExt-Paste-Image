  ...
  "src/config.js"(exports2, module2) {
    ...
    var variablePatterns = [
      ...
      },
      {
        ...
        get value() {
          return vscode2.workspace.workspaceFolders;
        }
      }
    ];
    var fileDirConfig;
    var baseDirConfig;
    function init() {
      fileDirConfig = vscode2.workspace.getConfiguration("mdPasteEnhanced")["path"];
      baseDirConfig = vscode2.workspace.getConfiguration("mdPasteEnhanced")["basePath"];
    }
...
var require_clipboard = __commonJS({
  "src/clipboard/clipboard.js"(exports2, module2) {
    var scriptDir = __dirname;
    var spawn = require("child_process").spawn;
    var path = require("path");
    var vscode2 = require("vscode");
    var fs = require("fs");
    var testImgScript = path.join(scriptDir, "winTestImage.ps1");
    var saveImgScriptName = "winPNGfromClipboard.ps1";
    var saveImgScript = path.join(scriptDir, saveImgScriptName);
    ...
    async function saveImage(fileDir, fileName) {
      ...
      if (fs.existsSync(filePath)) {
        let choose = await vscode2.window.showInformationMessage(
      ...
      if (platform == "win32") {
        ...
        ).on("error", (e) => vscode2.window.showInformationMessage(e.message));
        ...
        ).on("error", (e) => vscode2.window.showInformationMessage(e.message)).on(
          "close",
...
var require_main = __commonJS({
  "src/main.js"(exports2, module2) {
    ...
    async function main2() {
      let clipContent = await vscode2.env.clipboard.readText();
      if (clipContent == "" && await clipboard.isImage()) {
        try {
          pasteImage();
        } catch (e) {
          vscode2.window.showInformationMessage(e);
        }
      } else {
        if (vscode2.window.activeTextEditor.selection.isEmpty == false) {
          try {
            vscode2.commands.executeCommand("markdown.extension.editing.paste");
          } catch (e) {
            vscode2.commands.executeCommand("editor.action.clipboardPasteAction");
          }
        } else {
          vscode2.commands.executeCommand("editor.action.clipboardPasteAction");
        }
    ...
    function pasteImage() {
      ...
      render(config2.baseDir, filePath);
    }
